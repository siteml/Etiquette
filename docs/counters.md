...
| UD01 column   | Use                                          |
|---------------|----------------------------------------------|
| `Key1`        | counter key, e.g. `DUDEK`, `WHIRLPOOL` (coarse per-customer default) |
...
3. Client call (already implemented):
   `EpicorCounterProvider.ReserveAsync("DUDEK", 500)` →
   `POST /api/v2/efx/KHC/EtiqCounters/NextSerial` body
   `{"counter":"DUDEK","count":500}` → `{"next":100042}`.
...
- **Never reset** a production counter downward; duplicates on customer
  ...
  fork a NEW counter key (e.g. `WHIRLPOOL-2027`) and record it in the
  template.
