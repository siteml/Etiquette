    ...
    var t = EtiqTemplate.Parse("""
        <svg xmlns="http://www.w3.org/2000/svg" width="3in" height="1in" viewBox="0 0 288 96">
          <metadata><etiq:label xmlns:etiq="https://etiquette.dev/ns/0.1">
            ...
            <etiq:field name="Serial" source="serial" counter="ACME"/>
          ...
          <g data-layer="Fields">
            <text x="8" y="20" data-field="Tag" data-width="200" data-overflow="shrink">EX-SAMP-SP-000001</text>
...
Check("resolver: kitchen-sink compose resolves end-to-end", () =>
{
    ...
    var ctx = new ResolveContext
    {
        ...
        Rest = (conn, q, pick) => conn == "glpi" ? "MONTERREY" : null,
        Counters = new LocalFileCounterProvider(tmp),
        Now = new DateTime(2026, 8, 2),
    };
    var r = new FieldResolver(t, ctx);
    AssertEq("EX-WIDG-MO-2608-000001", r.Resolve("Tag"), "composed tag");
    File.Delete(tmp);
});
...
Check("resolver: field case= is opt-in, normalizes, validator gates enum", () =>
{
    ...
    var r = new FieldResolver(t, new ResolveContext
        { PromptValues = { ["Lot"] = "ab-12x", ["Note"] = "Mixed Case",
                           ["Co"] = "ACME and NORTHWIND mfg", ["Nm"] = "AsIs Text", ["Bad"] = "q" } });
    ...
    AssertEq("Acme and Northwind Mfg", r.Resolve("Co"), "English title: small word lowered, last word capped");
...
Check("pick lists: set behavior, default, no-selection blocks, validator", () =>
{
    ...
    var r = new FieldResolver(t, new ResolveContext
        { ListSelections = { ["ShipTo"] = "Monterrey Plant", ["Dept"] = "RCV" } });
    AssertEq("Monterrey Plant", r.Resolve("ShipName"), "name follows selection");
    AssertEq("Av. Ejemplo 100", r.Resolve("ShipAddr"), "addr follows same row");
    ...
    AssertEq("100 Example Ave", r2.Resolve("ShipAddr"), "default row");
    ...
    var t = EtiqTemplate.Parse("""
        <svg xmlns="http://www.w3.org/2000/svg" width="6in" height="4in" viewBox="0 0 6000 4000">
          <metadata><etiq:label xmlns:etiq="https://etiquette.dev/ns/0.1">
            ...
            <etiq:field name="Zip" source="fixed" value="62701"/>
...
Check("JsonPick: dotted path with index, misses return null", () =>
{
    using var doc = JsonDocument.Parse("""
        {"assets":[{"name":"PC-042","location":{"city":"SPRINGFIELD"}},{"name":"PC-043"}],
         "count":2,"tag":null}
        """);
    var root = doc.RootElement;
    AssertEq("PC-042", JsonPick.Evaluate(root, "assets[0].name")!, "index + prop");
    AssertEq("SPRINGFIELD", JsonPick.Evaluate(root, "assets[0].location.city")!, "nested");
...
Check("RestClient: GLPI session dance + pick via mock", () =>
{
    ...
    var handler = new MockHandler(req =>
    {
        ...
        return Json("""{"id":42,"name":"PC-042","locations_id":{"name":"SPRINGFIELD"}}""");
    });
    ...
    AssertEq("SPRINGFIELD", c.Fetch("Computer/42", "locations_id.name")!, "nested pick");
...
Check("EpicorClient BAQ filter mode", () =>
{
    var handler = new MockHandler(req =>
    {
        ...
        AssertEq("/SaaS999/api/v2/odata/EPIC01/BaqSvc/EX-JobLabel/Data", req.RequestUri!.AbsolutePath, "path");
...
Check("EpicorClient function call", () =>
{
    var handler = new MockHandler(req =>
    {
        ...
        AssertEq("/SaaS999/api/v2/efx/EPIC01/EtiqCounters/NextSerial", req.RequestUri!.AbsolutePath, "efx path");
        string body = req.Content!.ReadAsStringAsync().GetAwaiter().GetResult();
        Assert(body.Contains("\"counter\":\"ACME\""), "payload");
        return Json("""{"next":100042}""");
    });
    using var c = new EpicorClient(Cfg(), handler);
    var result = c.CallFunctionAsync("EtiqCounters", "NextSerial", new { counter = "ACME" })
        .GetAwaiter().GetResult();
...
Check("EpicorCounterProvider reserve + peek via mock", () =>
{
    var handler = new MockHandler(req =>
    {
        ...
        {
            Assert(body.Contains("\"counter\":\"ACME\"") && body.Contains("\"count\":500"), "payload: " + body);
            return Json("""{"next":100042}""");
        }
    ...
    AssertEq(100042L, p.ReserveAsync("ACME", 500).GetAwaiter().GetResult(), "reserve");
    AssertEq(100542L, p.PeekAsync("ACME").GetAwaiter().GetResult(), "peek");
...
{
    BaseUrl = "https://instance.epicorsaas.com/SaaS999/",
    Company = "EPIC01",
    ApiKey = "KEY",
    Username = "user",
    Password = "pw",
